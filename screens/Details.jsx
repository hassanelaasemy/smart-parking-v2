...
const DetailsScreen = ({ navigation, route }) => {
  ...
  const ManageFavoritekByid = () => {
    ...
    AsyncStorage.getItem("accessToken")
      .then((accees) => {
        axios
          .get(`http://192.168.11.105:8080/v2/parking/byid/${id}/favorite`, {
            headers: {
              Authorization: `Bearer ${accees ? accees : ""}`,
            },
          })
  ...
  const DisplayReviwsByid = () => {
    axios
      .get(`http://192.168.11.105:8080/v2/parking/byid/${id}/reviews`)
      .then((resopnse) => {
  ...
  const DisplayReviwsAuthByid = () => {
    AsyncStorage.getItem("accessToken")
      .then((accees) => {
        axios
          .get(
            `http://192.168.11.105:8080/v2/reviews/parking/find/${id}/byauth`,
            {
              headers: {
                Authorization: `Bearer ${accees ? accees : ""}`,
              },
    ...
    AsyncStorage.getItem("accessToken")
      .then((accees) => {
        ...
        if (comment != "") {
          axios
            .post(
              "http://192.168.11.105:8080/v2/reviews/parking/manage/byid",
              datasend,
              {
                headers: {
                  Authorization: `Bearer ${accees ? accees : ""}`,
